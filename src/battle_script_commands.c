...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_PEARL,           {   _,   1,   1,   1,   1,   4,   4,   5,   4,   5, } },
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    else if (((GetBattlerAbility(gBattlerAttacker) == ABILITY_NO_GUARD) || (GetBattlerAbility(gBattlerAttacker) == ABILITY_NIGHT_VISION))
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (gSideStatuses[battlerDef] & SIDE_STATUS_LUCKY_CHANT)
    {
        ...
        || (abilityAtk == ABILITY_FATAL_STRIKE))
    {
...
static void Cmd_tryKO(void)
{
    ...
    else
    {
        if ((((gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS)
            ...
            || GetBattlerAbility(gBattlerAttacker) == ABILITY_NIGHT_VISION
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (gChosenActionByBattler[gBattlerTarget] == B_ACTION_USE_MOVE
        ...
        && (
            ...
            || (GetBattlerAbility(gBattlerTarget) == ABILITY_ENSNARE)
            )
        )
    {
        s32 i;
...
static void Cmd_recoverbasedonsunlight(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        {
            if (GetBattlerAbility(gBattlerTarget) == ABILITY_CELESTIAL_BODY)
                gBattleMoveDamage = 20 * GetNonDynamaxMaxHP(gBattlerAttacker) / 30;
            else if (!(gBattleWeather & B_WEATHER_ANY) || !WEATHER_HAS_EFFECT || GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_UTILITY_UMBRELLA)
                gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / 2;
            else if (gBattleWeather & B_WEATHER_SUN)
                gBattleMoveDamage = 20 * GetNonDynamaxMaxHP(gBattlerAttacker) / 30;
